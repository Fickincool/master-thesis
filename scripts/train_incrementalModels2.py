...
for input_type in ['cryoCARE', 'rawCET']: 
    for nPatches in range(4, 34, 4):

        tb_logdir = os.path.join(PARENT_PATH, 'data/model_logs/incremental_models_2/logs/BaselineModel/%s/train%s/nPatches_%i' %(input_type, concat_train_ids, nPatches))
        ...
        nPatches_training=nPatches, num_workers=1, train_callbacks=callbacks)
