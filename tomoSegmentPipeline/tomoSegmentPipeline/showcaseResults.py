...
def load_model(model_file, Nclass, weight_decay=0):
    ...
    model = DeepFinder_model(Nclass, Tversky_loss, lr, weight_decay, pretrain_type=pretrain_type)
...
def save_classPred(class_pred, model_name, train_tomos, tomo_file, input_type, overwrite=True):

    class_pred = np.int16(class_pred)

    tomo_id = tomo_file.split('/')[-1].split('.')[0].replace('_0000', '')
    name_prediction_file = 'class1Pred_' + tomo_id + '.mrc'
    file_target = PARENT_PATH+'data/processed2/deepFinder/predictions/%s/%s/%s/' %(input_type, model_name, train_tomos)
...
def fullTomogram_modelComparison(model_fileList, n_centers_list, tomo_file, label_file, overwrite_prediction, is_model_data=True):
    ...
    for model_file, n_centers in zip(model_fileList, n_centers_list):
        ...
        if overwrite_prediction:
            ...
            try:
                class_pred = load_classPred(model_name, train_tomos, tomo_file, input_type)
