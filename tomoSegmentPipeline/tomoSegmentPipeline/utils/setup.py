...
RAW_TOMO_PATH = PARENT_PATH+'data/raw_cryo-ET/patch_creation/result/Task511_cryoET/imagesTr'
ISONET_TOMO_PATH = PARENT_PATH+'data/isoNet/RAW_dataset/RAW_corrected_i30/patch_creation/result/Task511_cryoET/imagesTr'
CRYOCARE_ISONET_TOMO_PATH = PARENT_PATH+'data/isoNet/cryoCARE_dataset/cryoCARE_corrected/patch_creation/result/Task511_cryoET/imagesTr'
...
def get_tomo_indices():
    ...
    tomo_ids, counts = np.unique([f.as_posix().split('/')[-1].split('_')[0] for f in files], return_counts=True)
    ...
    label_files = [f.as_posix() for f in Path(TOMO_PATH).glob('*tomo*') if 'lbl' in f.as_posix()]
...
def get_paths(tomo_names:list, input_type:str):
    ...
    if input_type=='cryoCARE':
        ...
        raise NotImplementedError('Only "isoNET", "cryoCARE+isoNET", "cryoCARE" and "rawCET" input type is implemented')
