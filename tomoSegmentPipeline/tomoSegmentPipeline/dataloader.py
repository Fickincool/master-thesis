...
class tomoSegment_dataset(Dataset):
    def __init__(self, path_data, path_target, dim_in, Ncl, Lrnd, augment_data):
        ...
        self.data_list, self.target_list = core.load_dataset(
            self.path_data, self.path_target, ""
        )
    ...
    def shift_coords(self, z, y, x, tomodim):
        ...
        if z < self.p_in:
            z = self.p_in
        if x > tomodim[2] - self.p_in:
            x = tomodim[2] - self.p_in
        if y > tomodim[1] - self.p_in:
            y = tomodim[1] - self.p_in
        if z > tomodim[0] - self.p_in:
            z = tomodim[0] - self.p_in
    ...
    def create_grid(self):
        ...
        for c in self.tomo_shape:
            centers.append(
                np.arange(
                    max(noise_margin + self.p_in // 2, self.p_in),
    ...
    def get_patch(self, index):
        ...
        # Get patch:
        patch_data = sample_data[
            z - self.p_in : z + self.p_in,
            y - self.p_in : y + self.p_in,
            x - self.p_in : x + self.p_in,
        ]
        patch_target = sample_target[
            z - self.p_in : z + self.p_in,
            y - self.p_in : y + self.p_in,
            x - self.p_in : x + self.p_in,
        ]
...
class tomoSegment_dummyDataset(Dataset):
    ...
    def create_dummyBatch(self, index):
        ...
        batch_data = transpose_to_channels_first(torch.tensor(batch_data)).to(
            torch.float
        )
        batch_target = transpose_to_channels_first(torch.tensor(batch_target)).to(
            torch.float
        )
...
class reconstructionTask_dataset(Dataset):
    ...
    def __init__(
        ...
        if not self.flag_direct_read:
            self.data_list, self.target_list = core.load_dataset(
                self.path_data, self.path_target, h5_dset_name
            )
    ...
    def createRandomSquare(self, mask):
        ...
        center = [
            np.random.randint(z_c_bounds[0], z_c_bounds[1]),
            np.random.randint(y_c_bounds[0], y_c_bounds[1]),
        ...
        mask[
            0,
            z_c - n_pixel_z : z_c + n_pixel_z,
            y_c - n_pixel_y : y_c + n_pixel_y,
            x_c - n_pixel_x : x_c + n_pixel_x,
        ] = 0
    ...
    def generate_batch_from_array(self, index):
        ...
        # Get patch position:
        x, y, z = core.get_patch_position(
            tomodim, self.p_in, self.objlist[index], self.Lrnd
        )
