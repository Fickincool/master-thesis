...
class Train(core.DeepFinder):
    def __init__(self, Ncl, dim_in, lr, weight_decay, Lrnd, tensorboard_logdir, model_name, augment_data,
     batch_size, epochs, pretrained_model=None):
        ...
        self.model = model.DeepFinder_model(self.Ncl, self.loss_fn, lr, weight_decay, self.pretrain_type) 
    ...
    def get_pretrain_type(self):
        ...
        if self.pretrained_model is None:
            ...
            raise ValueError("Pretrain type can only be: proxyLabel, reconstructionTask or None")
    ...
    def launch(self, train_tomos, val_tomos, input_type, num_gpus, accelerator, num_workers, nPatches_training=None, resume_from_checkpoint=None,
     train_callbacks=None, dataset=tomoSegment_dataset):
        """
        ...
        if dataset==tomoSegment_dataset:
            train_set = dataset(paths_trainData, paths_trainTarget, self.dim_in, self.Ncl, self.Lrnd, self.augment_data)
            # Note: augmentation is off for the validation set
            val_set = dataset(paths_valData, paths_valTarget, self.dim_in, self.Ncl, 0, False)
        ...
        val_loader = DataLoader(val_set, batch_size=self.batch_size, shuffle=True, pin_memory=True,
         num_workers=num_workers)

        logger = pl_loggers.TensorBoardLogger(self.tensorboard_logdir, name='',  default_hp_metric=False)

        trainer = pl.Trainer(logger=logger, log_every_n_steps=1, gpus=num_gpus, max_epochs=self.epochs,
         enable_progress_bar = False, accelerator=accelerator, resume_from_checkpoint=resume_from_checkpoint,
        ...
        if trainer.is_global_zero:
            ...
            self.write_modelSummary(modelSummary_folder, train_patchCounts, val_patchCounts, input_type)
    ...
    def save_model(self):
        ...
        name_model = '%s_ep%i_in%i_lr%f_%s.model' %(self.model_name, self.epochs, self.dim_in, self.lr, version)
    ...
    def write_modelSummary(self, modelSummary_folder, train_set, validation_set, input_type):
        """
        ...
        row_data = [self.model_name, logdir_path, input_type, train_set, validation_set, self.augment_data, self.Lrnd, self.model.lr,
                    self.batch_size, self.dim_in, epochs_str,
                    self.pretrain_type, self.pretrained_model, self.loss_fn, best_val_loss_epoch, associated_train_loss_epoch,
                    associated_val_class1_dice, best_val_loss_epoch_idx]
