...
class Segment(core.DeepFinder):
    ...
    def launch(self, dataArray):
        ...
        dataArray = (dataArray[:] - np.mean(dataArray[:])) / np.std(
            dataArray[:]
        )  # normalize
        dataArray = np.pad(
            dataArray, self.pcrop, mode="constant", constant_values=0
        )  # zeropad
        dim = dataArray.shape
        ...
        for x in pcenterX:
            for y in pcenterY:
                for z in pcenterZ:
                    ...
                    patch = np.reshape(
                        patch, (1, self.P, self.P, self.P, 1)
                    )  # reshape for keras [batch,x,y,z,channel]
...
class Cluster(core.DeepFinder):
    ...
    def launch(self, labelmap):
        ...
        Nclass = (
            len(np.unique(labelmap)) - 1
        )  # object classes only (background class not considered)
        ...
        start = time.time()
        clusters = MeanShift(bandwidth=self.clustRadius, bin_seeding=True).fit(
            objvoxels
        )
        end = time.time()
        ...
        for c in range(Nclust):
            ...
            for m in range(clustSize):  # get labels of cluster members
                ...
                clustMember.append(
                    labelmap[
                        clustMemberCoords[0], clustMemberCoords[1], clustMemberCoords[2]
                    ]
                )
            ...
            objlist = ol.add_obj(
                objlist, label=winninglabel, coord=centroid, cluster_size=clustSize
            )
